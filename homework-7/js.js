...
// в объекте map, в методе render была опечатка при пуше в массив usedCells foodPoint
...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    this.usedCells.push(foodCell); // была опечатка foodPoint
  },
};
...
const snake = {
   ...
   * @param {int} rows Максимальное количество ячеек по Х
   * @param {int} cols Максимальное количество ячеек по Y
   */
  ...
  makeStep() {
    ...
    // создадим переменную головы для удобства и положим туда след.точку при шаге
    let nextHeadPoint = this.getNextStepHeadPoint();
    // проверка будет ли следующая точка за пределами поля, если да, то отзеркалим змейку
    if([nextHeadPoint].some(headPoint => headPoint.x === this.maxX)){
      nextHeadPoint = {x: 0, y: nextHeadPoint.y};
    } else if([nextHeadPoint].some(headPoint => headPoint.x === -1)){
      nextHeadPoint = {x: this.maxX - 1, y: nextHeadPoint.y};
    } else if([nextHeadPoint].some(headPoint => headPoint.y === this.maxY)){
      nextHeadPoint = {x: nextHeadPoint.x, y: 0};
    } else if([nextHeadPoint].some(headPoint => headPoint.y === -1)){
      nextHeadPoint = {x: nextHeadPoint.x, y: this.maxY - 1};
    }
 ...
 * @property {HTMLElement} countEl DOM-элемент для вставки числа
 отображающего
...
const game = {
  ...
  reset() {
    ...
    this.snake.init(this.getStartSnakeBody(), 'up', this.getMaxX(), this.getMaxY());
  ...
  /**
   * Возвращает количество строк, заданных в конфиге
   * @returns {int} для получения Max X
   */
  getMaxX() {
    return this.config.getRowsCount();
  },

  /**
   * Возвращает количество колонок, заданных в конфиге
   * @returns {int} для получения Max Y
   */
  getMaxY() {
    return this.config.getColsCount();
  },
